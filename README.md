...
3. 문제 사진 위에 **펜**으로 풀이를 쓰고 **지우개**로 지우며 풀이, 필요시 전체 지우기 (펜 굵기/색상 조절 가능). 창 가장자리를 끌면 필기 공간만 늘어나고 사진은 그대로 유지되며, 두 손가락 핀치로 사진 자체를 확대/축소
...
(운영 DB에 대고 돌리면 안 됩니다). CI(`.github/workflows/ci.yml`)는 push/PR마다
임시 Postgres 컨테이너를 띄워 같은 테스트와 관리자 대시보드 빌드를 자동으로
실행합니다.
