...
1. `admin-server`(`http://localhost:4000`)에 로그인 → 문제 제목, 난이도(상/중/하), 문제 유형(객관식/주관식) + 정답(선택, 자동채점용), 문제 사진, (선택) 설명/해설을 입력해 등록
2. `main-server`(`http://localhost:3000`)에서 난이도 카드를 누르면 그 난이도의 문제가 **셔플백(shuffle-bag) 알고리즘으로 랜덤 진입** — 등록된 문제를 전부 한 바퀴 보여주기 전까지는 같은 문제가 다시 나오지 않음. 특정 문제를 직접 고르고 싶으면 풀이 화면 상단의 목록 아이콘으로 이동 가능
3. 문제 사진 위에 **펜**으로 풀이를 쓰고 **지우개**로 지우며 풀이, 필요시 전체 지우기 (펜 굵기 조절 가능)
4. 정답이 등록된 문제는 화면 하단에 객관식 5지선다 버튼 또는 주관식 입력창이 나타나고, 채점하면 정답/오답과 정답을 바로 보여줌 (정답 값 자체는 목록/상세 API 응답에 노출되지 않고, 채점 API를 통해서만 서버가 비교)
5. 문제 화면 상단에서 **확대/축소**로 사진을 크게 보고, **전체화면** 버튼으로 큰 화면(행사/발표용)에 꽉 채워서 볼 수 있음, **다음 문제** 버튼으로 같은 난이도의 다음 랜덤 문제로 이동
