...
- 마지막 10초 초읽기, 시간 종료 부저, 정답 팡파레, 축하 색종이. 소리는 끌 수 있습니다.
- 같은 회차(최근 12시간)에 이미 나온 문제는 다시 뽑히지 않습니다.
  난이도 랜덤이 기본이지만 **문제를 직접 골라서** 낼 수도 있습니다.
- 정답자 이름과 상품이 기록되고, 오늘의 명예의 전당과 **반 대항 순위**에 쌓입니다.
  이름을 `2-3 김민수`로 넣으면 앞부분을 반으로 잡습니다.
...
- **와이파이가 끊겨도 진행이 멈추지 않습니다.** 정답자 기록은 브라우저에 쌓아두고
  네트워크가 돌아오면 자동으로 보냅니다. 화면 오른쪽 위에 "저장 대기 N건"이 뜹니다.
  새로고침하거나 칠판이 꺼졌다 켜져도 남아 있습니다.
...
2. **강당에서** — 전자칠판 브라우저로 `/admin`을 열고 상단의 **🎤 행사 진행**을 누릅니다
   (주소를 바로 치려면 `/admin/show.html`). 처음 한 번 로그인하면 그 뒤로는 바로 열립니다.
...
학생을 누르면 **그 학생이 칠판에 실제로 쓴 풀이**를 그대로 다시 볼 수 있습니다 —
성공률 숫자만으로는 왜 틀렸는지 알 수 없기 때문입니다.

> 학생 이름이 실려 있는 공개 페이지(`/`)에는 검색엔진 색인을 막아뒀습니다(`noindex`).
> 강당에서 부르는 이름이라도 인터넷에 색인되는 건 다른 문제라서입니다.

## 폴더 구조

```
shared/db.js                          # 문제/행사기록 Postgres 모듈 (두 서버가 공유)
shared/class-ranking.js               # 반 대항 집계 (진행 화면과 공개 페이지가 같은 순위를 쓰도록)
